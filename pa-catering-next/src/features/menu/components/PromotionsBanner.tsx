  ...
  return (
    <div className="bg-black/50 blur-[0.1px] backdrop-blur-md p-4 sticky bottom-0 left-0 w-full z-10 border-t border-white/20">
      <div className="w-full max-w-3xl mx-auto flex flex-col items-center">
        <div className={`transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`} key={index}>
          <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-4">
            <div className="flex-shrink-0">{promo.icon}</div>
            <div>
              <h3 className="text-xl font-bold text-white" style={{ filter: 'drop-shadow(0 0 3px #ff0000)' }}>
                {promo.title}
              </h3>
